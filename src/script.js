...
async function fetchData(source) {
  if (source === 'json') {
    ...
    const enemiesResponse = await fetch('json/enemies.json')
    const skillsResponse = await fetch('json/skills.json')
    const worldsResponse = await fetch('json/worlds.json')
    ...
    skills: [
      {
        ...
        rank: 'C',
        type: 'Offensive',
        effect: 'Deals moderate fire damage.',
      },
      {
        name: 'Regeneration',
        rank: 'B',
      ...
      },
      {
        ...
        rank: 'D',
        type: 'Defensive',
        effect: 'Increases defense for a short duration.',
      },
    ],
    worlds: {
      ForestWorld: {
...
function updateEnemy() {
  ...
  if (
    currentAreaEnemies.length > 0 &&
    currentEnemyIndex < currentAreaEnemies.length
  ) {
...
document.getElementById('consume-enemy').addEventListener('click', () => {
  if (currentAreaEnemies.length > 0) {
    // ✅ Fix: Use `currentAreaEnemies` instead of `enemies`
